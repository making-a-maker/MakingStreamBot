...
import chat_listener.listener as chat_listener
import command_processor.commander as commander
from common.ThreadCommonObject import ThreadCommonObject
...
thread_count = 0
for i in config["thread_enable"]:
    thread_count += 1 if i else 0

ready = threading.Barrier(thread_count + 1, timeout=10)
# ready = ""
logger.debug("Creating threads")

chat_thread = chat_listener.ChatListener(tco, tco_lock, ready)
